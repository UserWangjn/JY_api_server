...
def top_add_ceshi(fun):
    ...
    def top_add():
        ...
        if request.form['type'] == 'top_add':
            ...
            cu.executemany('INSERT INTO  xiangmu_data values (null,?,?,?,?)', [
             (name, request.form['name'],
              request.form['request_url'], str(time.time()))])
        else:
            if 'top_bianji' in request.form['type']:
...
def delete_moke_xiangmu(fun):
    ...
    def delete_moke_xiangmu():
        ...
        if request.form['type'] == 'top_delete':
            ...
            if request.form['type'] == 'yewu_delete':
                cu.execute('delete from yewu_data where id="%s"' % request.form['id'])
            else:
                if request.form['type'] == 'case_delete':
...
def yewuadd(func):
    ...
    def yewuadd():
        ...
        db = sqlite3.connect(current_app.config.get('DB_DIZHI'))
        cu = db.cursor()
        user = cu.execute('select name from user where ip="%s" order by time desc limit 0,1' % request.headers.get('X-Real-IP')).fetchall()[0][0]
        db.close()
        name = request.form['name']
        id = request.form['id']
