...
def send_emali(func):
    ...
    def aaddd():
        ...
        email_address = [ i for i in cu.execute('select email from run where name="%s" order  by time desc limit 0,1 ' % name).fetchall()[0][0].split('##') if i.strip() != ''
                        ]
