...
class reead_excel(object):
    ...
    def read_case(self):
        for i in self.path:
            ...
            if 'before_request' in sheet2.row_values(0):
                self.all_case[i] = dict(zip(self._all_id, sheet2.col_values(sheet2.row_values(0).index('before_request'))[1:]))
    ...
    def yewuliu(self):
        for path, detail in self.all_case.items():
            if detail['before_request'] == False:
                ...
                detail.pop('before_request')
                self._yewu = []
                for case_id, before_detail in detail.items():
                    self._statu = 0
    ...
    def find_jiekou(self, before_detail, yewu_append):
        ...
        if before_path in self.all_case.keys() and int(self._par_id) in self.all_case[before_path].keys():
