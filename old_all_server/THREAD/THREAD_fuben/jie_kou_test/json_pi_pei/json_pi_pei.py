        ...
        for k in v.keys():
            ...
            if type(v[k]) != dict and type(v[k]) != list:
                if k not in j:
                    v.pop(k)
                    continue
                if type(j[k]) not in [float, int, long] and j[k].strip() == '':
                    continue
                s = change(v[k], j[k])
                v[k] = s
            elif type(v[k]) == list:
        ...
        for k in v.keys():
            if v[k] == 'change_json':
                ...
                if k not in j:
                    ...
                    if type(j[k]) not in [float, int, long] and j[k] != None:
                        try:
            ...
            for k, i in enumerate(a):
                if type(a[k]) in [list, dict]:
                    ...
                    if type(a[k]) == int:
                        ...
                        b[k] == bool(b[k])
                    elif type(a[k]) in [str, unicode] and a[k] != 'change_json':
                        b[k] == str(b[k])

        elif type(a) == dict:
            for k in a:
                if type(b) == dict and k in b.keys():
                    if type(a[k]) in [list, dict]:
                        json_change(a[k], b[k])
                    else:
                        if type(b[k]) == 'float' and b[k] == int(b[k]):
                        ...
                        if type(a[k]) == int:
                            ...
                            b[k] == str(b[k])
                        elif type(a[k]) in [str, unicode] and a[k] != 'empty_data':
                            b[k] == ''
...
def change_json_data(v, excel_data):
    if v != '':
        ...
        if type(v) == dict:
            for k in v:
                if type(v[k]) not in [str, unicode]:
