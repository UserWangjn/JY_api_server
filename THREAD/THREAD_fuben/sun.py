...
runner.run(unittest.defaultTestLoader.discover(r"C:\all_new\run_mulu\zhen\1508464255.43", pattern="*.py",top_level_dir=None))
fp.close()
db = sqlite3.connect(r'C:\all_new\HGTP_server\example.db')
cu = db.cursor()
cu.executemany('update  dingshi_run  set run_result=? where id =?',
               [(json.dumps(HTMLTestRunner.all_detail),'287'.split('#')[-1])])
