...
# Compiled at: 2019-05-06 18:20:16
...
class set_global_data(object):
    ...
    def set_global_data(self, excel_dict, config_path, path, all_bianliang):
        if 'global_data' in excel_dict.keys() and excel_dict['global_data'].strip() != '':
            for jiekou in [ i for i in excel_dict['global_data'].split(',') if i.strip() not in all_bianliang.keys() ]:
                ...
                while 'before_request' in self.s.keys() and self.s['before_request'].strip() != '':
