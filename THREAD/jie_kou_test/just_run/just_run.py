        ...
        if self.url != {}:
            if self.config_path.get('config', 'method') == 'post':
                if self.config_path.get('config', 'head_key').strip() == '':
                    ...
                    try:
                        self.respons = json.loads(port.post(self.req, self.config_path, self.url)['respons'])
                    except:
                ...
                if self.config_path.get('config', 'head_key').strip() == '':
                    ...
                    try:
                        self.respons = json.loads(port.get(self.req, self.config_path, self.url)['respons'])
                    except:
                        self.respons = {'result': port.get(self.req, self.config_path, self.url)['respons']}

            elif self.config_path.get('config', 'method') == 'delete':
                try:
                    self.respons = json.loads(port.delete(self.req, self.config_path, self.url)['respons'])
                except:
