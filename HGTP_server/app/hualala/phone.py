...
# [GCC 9.1.0]
# Embedded file name: C:\jieyuelianhe\old_all_server\HGTP_server\app\hualala\phone.py
# Compiled at: 2019-02-15 18:00:28
...
def shouye(func):
    ...
    def shouye():
        ...
        if user in current_app.config.get('ZICHAN_QUANXIAN').split(','):
            ...
            user_statu = 0
        return render_template('/phoine_guanli/phone.html', phone_detail=phone_detail, all_pinpai=all_pinpai, all_xitong=all_xitong, all_chicun=all_chicun, all_fenbianlv=all_fenbianlv, user_statu=user_statu, today=riqi)
...
def phone_simple_caozuo(func):
    ...
    def phone_simple_caozuo(name):
        ...
        if name == 'guihuan':
            ...
            if detail[0][0] == '已出借':
                riqi = time.strftime('%Y-%m-%d', time.localtime())
                cu.execute('update phone_detail set  statu="未出借",guihuanriqi=? where id=?', (
                 riqi, id))
                db.commit()
                db.close()
            ...
            if name == 'yuding':
                ...
                try:
                    ...
                    if detail[0][0] == '未出借':
                        riqi = time.strftime('%Y-%m-%d', time.localtime())
                        cu.execute('update phone_detail set  statu="已预订",guihuanriqi=?,jieyongrenyuan=?,jieyongriqi=? where id=?', (
                ...
                if name == 'chujie':
                    ...
                    if detail[0][0] == '已预订':
                        riqi = time.strftime('%Y-%m-%d', time.localtime())
                        cu.execute('update phone_detail set  statu="已出借",jieyongriqi=? where id=?', (
                         riqi, id))
                        db.commit()
                        db.close()
                        return jsonify(statu='success', riqi=riqi)
